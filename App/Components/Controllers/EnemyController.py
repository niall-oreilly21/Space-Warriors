...
from Engine.GameObjects.Components.Physics.WaypointFinder import WaypointSystem
...
class EnemyController(Component):
    ...
    def start(self):
        ...
        self.__waypoint_system = self._parent.get_component(WaypointSystem)
    ...
    def calculate_patrol_routes(self, enemy_position, game_time):
        ...
        if self.__waypoint_system.has_reached_waypoint(enemy_position):
            self.__waypoint_system.move_to_next_waypoint()

        # Calculate the interpolation factor based on the distance to the current waypoint
        current_waypoint = self.__waypoint_system.get_current_waypoint()
        ...
        direction.normalize()
        target_velocity = direction * self.__speed * 0.1
